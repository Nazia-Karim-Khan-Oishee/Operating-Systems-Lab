...
void preemptivePriorityScheduling(int n, int arrivalTimes[], int burstTimes[], int startTimes[], int finishTimes[], int priorities[], int hasArrived[], int currentProcesses[], int remainingTime[]) {
    int totalTime = 0;
    for (int i = 0; i < n; i++) {
    ...
    for (int currentTime = 0; currentTime <= totalTime; currentTime++) {
        for (int j = 0; j < n; j++) {
            if (arrivalTimes[j] <= currentTime && !hasArrived[j]) {
                hasArrived[j] = 1;
                remainingTime[j] = burstTimes[j];
            }
        }
        int maxPriority = -1;
        int maxIndex = -1;
        ...
        if (maxIndex != -1) {
            if (startTimes[maxIndex] == -1) {
                startTimes[maxIndex] = currentTime;
            }
            remainingTime[maxIndex]--;
            if (remainingTime[maxIndex] == 0) {
                finishTimes[maxIndex] = currentTime + 1;
                priorities[maxIndex] = -1;
            }
            if (currentProcesses[queuePointer] != maxIndex + 1) {
                currentProcesses[queuePointer + 1] = maxIndex + 1;
                queuePointer++;
            } else {
                currentProcesses[queuePointer] = maxIndex + 1;
            }
        } else {
            break;
        }
...
int main() {
    ...
    preemptivePriorityScheduling(N, arrivalTimes, burstTimes, startTimes, finishTimes, priorities, hasArrived, currentProcesses, remainingTime);
